...
class TestIPA(BaseTestCase):
    ...
    def test_list_amministrazioni(self):
        response = self.client.open(
            "/ipa/v0/amministrazioni?limit=1", method="GET"
        )
        self.assert200(
        ...
        for related_link in ("uffici", "aree_organizzative_omogenee"):
    ...
    def test_list_uffici(self):
        response = self.client.open(
            "/ipa/v0/amministrazione/c_e472/uffici?limit=2", method="GET"
        )
        self.assert200(
            response, "Response body is : " + response.data.decode("utf-8")
        )
    ...
    def test_get_pa(self):
        response = self.client.open(
            "/ipa/v0/amministrazione/c_e472", method="GET"
        )
        self.assert200(
            response, "Response body is : " + response.data.decode("utf-8")
        )
    ...
    def test_get_ufficio(self):
        response = self.client.open(
            "/ipa/v0/amministrazione/c_e472/ufficio/KF05EG", method="GET"
        )
        self.assert200(
            response, "Response body is : " + response.data.decode("utf-8")
        )
